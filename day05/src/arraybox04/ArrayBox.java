...
public class ArrayBox implements Box {
    ...
    public int get(int index) {
        // 找小D同学 检测给定的index是否合法
        this.rangeCheck(index);
        // 上述代码可以执行 证明index是合法的
        // 则 找到index对应位置的元素 将其返回
    ...
    public int remove(int index) {
        ...
        for (int i = index; i < size - 1; i++) {// 从index位置开始至size-1结束 后面的元素依次前移覆盖
            elementData[i] = elementData[i + 1];
        }
        // 手动将最后的元素删除 让 size 减少一个记录
        elementData[--size] = 0;
        // 将旧数据返回
        return oldValue;
    }
